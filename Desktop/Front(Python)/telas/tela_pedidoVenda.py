...
from janelas.cd_pedidoVenda import cdPedidoVenda
...
class pedidoVenda:
    ...
    def __init__(self,root,main_instance):
        ...
        pesq_label = ctk.CTkLabel(pesquisar_frame, text="Pesquisar Venda:", font=("Arial", 14))
        ...
        self.pesq_entry.bind("<KeyRelease>", self.vendas_lista)

        btn_cdInsumo = ctk.CTkButton(pesquisar_frame, text='Cadastrar Venda', font=('Arial', 15, 'bold'), corner_radius=3, width=150, height=40,
    ...
    def vendas_lista(self,event=None):
        vendas = Access.listarpedidosVenda()
        ...
        for i in vendas:
            if (termoPesq in i['cliente']['razaoSocial'].lower() or
                termoPesq in i['semente']['nome'].lower()):
                
                venda_frame = ctk.CTkFrame(self.lista_frame, corner_radius=10)
                venda_frame.pack(fill="x", padx=10, pady=5)

                venda_label = ctk.CTkLabel(venda_frame, text=f"Cliente: {i['cliente']['razaoSocial']} | Quantidade: {i['qtd']} | Semente: {i['semente']['nome']}", font=("Arial", 14))
                venda_label.pack(side="left",pady=5)
                
                venda_label.bind("<Button-1>", lambda e, dados=i: self.abrir_tela_edicao(dados,0))
                
        if not vendas:
            msg_label = ctk.CTkLabel(self.lista_frame, text="Nenhum pedido de venda encontrado.", font=("Arial", 14))
            msg_label.pack(pady=5)
    
    def abrir_tela_edicao(self, dados,n):
        cdPedidoVenda(self, dados=dados,editar=n)
            
    def modificar_pedido(self, callback):
        result = Access.verificar_permissoes(self,0)
        if result:
            cdPedidoVenda(callback)
        else:
